...
- **Author Filter**: A dropdown in the toolbar lets you filter commits by author — select any name to show only their commits, or choose "All Authors" to clear the filter. The author list covers all authors reachable from HEAD (not just the currently loaded commits), so no one is missed even in long histories.
