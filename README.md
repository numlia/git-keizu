...
- **Remote Branch Actions**: Right-click a remote branch label to delete it on the remote, merge it into the current branch, or check it out as a new local branch. Checkout only ever creates a new branch that tracks the remote branch — if the entered name already belongs to an existing local branch, Git Keizu tells you so and leaves that branch untouched.
