...
| ------- | -------------------------------------------------- | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| TC-006  | stash !== null のコミット行を右クリック            | Normal - standard                                                          | スタッシュ専用コンテキストメニューが表示される（Apply, Pop, セパレータ, More（Create Branch, Drop）, セパレータ, Copy Name, Copy Hash の7要素） | 通常コミットメニューではないこと |
...
| TC-453  | stashバッジを行内と一覧の複製の両方でcontextmenu                                                               | Normal - スタッシュラベルの振分け                                          | `buildStashContextMenuItems` が行内・一覧で各1回、同じ `(TEST_REPO, hash, selector, 元の .commit 行)` で呼ばれ、`showContextMenu` の第3引数だけが各ラベル（行内ラベル／一覧の複製）。`buildRefContextMenuItems` は0回 | 059-01（2026-09-25）で契約を更新。旧契約の証跡は下の実行証跡を参照。AC-14 |
...
| スタッシュラベルの誤振分け（参照builderへ流れる） | TC-453                                                                         |
| 特殊文字の二重復号                                | TC-454                                                                         |
...
- 2026-09-24（059-02当時の結果）: TC-453 は `buildStashContextMenuItems`（実装を呼ぶspy）の呼出し0回と、行内・一覧で同じ builder・引数（第3引数を除く）であることを確認
- 2026-09-25（Feature 059-01）: TC-453 の契約を「stash builderが行内・一覧で各1回、同じrepo/hash/selector/元の行を受け、表示元だけが各ラベル、参照builderは0回」へ更新。テストの更新と実行証跡は対応プランのTask 2/3で記録する（追加観点は `02-context-menu-02.md` の S61）
