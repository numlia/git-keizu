...
| Case ID | Input / Precondition                           | Perspective (Equivalence / Boundary) | Expected Result                                | Notes                 |
| ------- | ---------------------------------------------- | ------------------------------------ | ---------------------------------------------- | --------------------- |
| TC-016  | multipleAllowed=true でコンストラクタ呼び出し  | Equivalence - normal                 | マルチセレクトモードが有効になる               | -                     |
| TC-017  | multipleAllowed=false でコンストラクタ呼び出し | Equivalence - normal                 | 単一選択モード（既存動作）が維持される         | 後方互換              |
| TC-018  | multipleAllowed=true で render() 実行          | Equivalence - normal                 | 各オプションにチェックボックス要素が描画される | input type="checkbox" |
| TC-019  | multipleAllowed=false で render() 実行         | Equivalence - normal (no checkbox)   | チェックボックス要素が描画されない             | 既存動作維持          |
...
| Case ID | Input / Precondition                                 | Perspective (Equivalence / Boundary) | Expected Result                                       | Notes        |
| ------- | ---------------------------------------------------- | ------------------------------------ | ----------------------------------------------------- | ------------ |
| TC-020  | "Show All"（インデックス 0）をクリック               | Equivalence - normal                 | 全ての個別チェックボックスが解除される                | -            |
| TC-021  | 個別オプションをクリック                             | Equivalence - normal                 | "Show All" が解除され、クリックしたオプションがトグル | -            |
| TC-022  | 全ての個別選択を 1 つずつ解除                        | Boundary - all deselected            | 自動的に "Show All" が選択状態に復帰する              | 自動復帰     |
| TC-023  | "Show All" 選択中に個別オプションを 1 つクリック     | Equivalence - normal                 | "Show All" が解除され、個別オプションがチェックされる | 排他切替     |
| TC-024  | 複数の個別オプションが選択中に "Show All" をクリック | Equivalence - normal                 | 全個別選択が解除され、"Show All" のみ選択             | 一括リセット |
...
| Case ID | Input / Precondition                                | Perspective (Equivalence / Boundary) | Expected Result                              | Notes              |
| ------- | --------------------------------------------------- | ------------------------------------ | -------------------------------------------- | ------------------ |
| TC-025  | ドロップダウン open → 選択変更あり → close          | Equivalence - normal                 | コールバックが発火し、選択値の配列が渡される | -                  |
| TC-026  | ドロップダウン open → 選択変更なし → close          | Boundary - no change                 | コールバックが発火しない                     | 不要なリロード防止 |
| TC-027  | "Show All" 選択状態で close                         | Equivalence - normal                 | コールバックに空配列が渡される               | 空配列 = Show All  |
| TC-028  | 1 項目のみ選択状態で close                          | Equivalence - normal (single)        | コールバックに 1 要素の配列が渡される        | -                  |
| TC-029  | 3 項目選択状態で close                              | Equivalence - normal (multiple)      | コールバックに 3 要素の配列が渡される        | -                  |
...
| Case ID | Input / Precondition        | Perspective (Equivalence / Boundary) | Expected Result                                | Notes      |
| ------- | --------------------------- | ------------------------------------ | ---------------------------------------------- | ---------- |
| TC-031  | "Show All" が選択中         | Equivalence - normal                 | 表示ラベルが "Show All" の名前になる           | -          |
| TC-032  | 個別オプション 1 件が選択中 | Equivalence - normal (single)        | 表示ラベルがそのオプション名になる             | -          |
| TC-033  | 個別オプション 2 件が選択中 | Equivalence - normal (multi)         | 表示ラベルが件数表示になる（例: "2 selected"） | -          |
...
| Case ID | Input / Precondition                       | Perspective (Equivalence / Boundary) | Expected Result                             | Notes                |
| ------- | ------------------------------------------ | ------------------------------------ | ------------------------------------------- | -------------------- |
| TC-035  | マルチセレクト: オプションクリック         | Equivalence - normal                 | ドロップダウンが閉じない（stopPropagation） | 単一選択との差異     |
| TC-036  | 単一選択: オプションクリック               | Equivalence - normal                 | ドロップダウンが閉じる（既存動作維持）      | 後方互換             |
| TC-037  | マルチセレクト: フィルタ入力で文字列を入力 | Equivalence - normal                 | オプションがフィルタテキストで絞り込まれる  | 既存フィルタ機能維持 |
