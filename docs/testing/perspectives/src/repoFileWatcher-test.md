...
| ------- | -------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------ | ----- |
| TC-001  | repoChangeCallback=vi.fn() | Equivalence - normal                 | RepoFileWatcherインスタンスが生成される。内部状態: fsWatcher=null, muted=false, resumeAt=0 | -     |
...
| Case ID | Input / Precondition                                | Perspective (Equivalence / Boundary) | Expected Result                                                                                                                    | Notes                     |
| ------- | --------------------------------------------------- | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| TC-002  | repo="/path/to/repo", fsWatcher=null (初回呼び出し) | Equivalence - normal                 | createFileSystemWatcher が `/path/to/repo/**` で1回呼ばれる。onDidCreate, onDidChange, onDidDelete の3イベントリスナーが登録される | L26-29                    |
| TC-003  | repo="/new/repo", fsWatcher=既存 (再呼び出し)       | Equivalence - normal (branch)        | 既存watcherのdispose()が1回呼ばれた後、新しいwatcherが `/new/repo/**` で作成される                                                 | L21 true分岐              |
...
| Case ID | Input / Precondition             | Perspective (Equivalence / Boundary) | Expected Result                                                  | Notes         |
| ------- | -------------------------------- | ------------------------------------ | ---------------------------------------------------------------- | ------------- |
| TC-006  | fsWatcher=監視中のwatcher        | Equivalence - normal                 | fsWatcher.dispose()が1回呼ばれる。fsWatcherがnullに設定される    | L34-35        |
| TC-007  | fsWatcher=null (未開始/停止済み) | Equivalence - abnormal               | 何も実行されない。エラーが発生しない                             | L33 false分岐 |
...
| Case ID | Input / Precondition   | Perspective (Equivalence / Boundary) | Expected Result             | Notes |
| ------- | ---------------------- | ------------------------------------ | --------------------------- | ----- |
| TC-009  | 初期状態 (muted=false) | Equivalence - normal                 | mutedプロパティがtrueになる | L40   |
...
| Case ID | Input / Precondition             | Perspective (Equivalence / Boundary) | Expected Result                                                   | Notes                         |
| ------- | -------------------------------- | ------------------------------------ | ----------------------------------------------------------------- | ----------------------------- |
| TC-010  | mute()呼び出し後                 | Equivalence - normal                 | mutedがfalseになる。resumeAtが現在時刻+1500ms付近の値に設定される | L44-45                        |
...
| Case ID | Input / Precondition                                         | Perspective (Equivalence / Boundary) | Expected Result                                                                        | Notes             |
| ------- | ------------------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------- | ----------------- |
| TC-012  | muted=true, uri=マッチする有効なパス                         | Equivalence - abnormal               | 早期リターン。repoChangeCallbackが呼ばれない                                           | L49               |
| TC-013  | muted=false, 現在時刻 < resumeAt (猶予期間内)                | Equivalence - abnormal               | 早期リターン。repoChangeCallbackが呼ばれない                                           | L51               |
| TC-014  | muted=false, 現在時刻 == resumeAt (境界値ちょうど)           | Boundary - exact                     | 早期リターンしない。`<` は厳密比較のため等値は通過し、コールバックがスケジュールされる | L51: `<` not `<=` |
| TC-015  | muted=false, 現在時刻 > resumeAt (猶予期間後), マッチするURI | Equivalence - normal                 | 猶予期間チェックを通過し、コールバックがスケジュールされる                             | L51通過           |
...
| Case ID | Input / Precondition                                         | Perspective (Equivalence / Boundary) | Expected Result                                                                                                         | Notes                       |
| ------- | ------------------------------------------------------------ | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- | --------------------------- |
| TC-016  | uri path=".git/config" (replace後)                           | Equivalence - normal                 | パターン1にマッチ。コールバックがスケジュールされる                                                                     | Git設定ファイル             |
| TC-017  | uri path=".git/refs/heads/main" (replace後)                  | Equivalence - normal                 | パターン1にマッチ。コールバックがスケジュールされる                                                                     | ブランチ参照                |
| TC-018  | uri path="src/index.ts" (replace後)                          | Equivalence - normal                 | パターン2にマッチ。コールバックがスケジュールされる                                                                     | 一般ソースファイル          |
| TC-019  | uri path=".gitignore" (replace後)                            | Equivalence - normal                 | パターン3にマッチ。コールバックがスケジュールされる                                                                     | .gitドットファイル          |
| TC-020  | uri path=".git/objects/ab/cd1234" (replace後)                | Equivalence - abnormal               | どのパターンにもマッチしない。早期リターン                                                                              | L50: Gitオブジェクト        |
| TC-021  | uri path=".git/hooks/pre-commit" (replace後)                 | Equivalence - abnormal               | どのパターンにもマッチしない。早期リターン                                                                              | L50: Gitフック              |
| TC-022  | uri path=".git/logs/HEAD" (replace後)                        | Equivalence - abnormal               | どのパターンにもマッチしない。早期リターン                                                                              | L50: Gitログ                |
| TC-023  | uri path=".git/COMMIT_EDITMSG" (replace後)                   | Equivalence - abnormal               | どのパターンにもマッチしない。早期リターン                                                                              | L50: コミットメッセージ編集 |
...
| Case ID | Input / Precondition                    | Perspective (Equivalence / Boundary) | Expected Result                                                                     | Notes              |
| ------- | --------------------------------------- | ------------------------------------ | ----------------------------------------------------------------------------------- | ------------------ |
| TC-028  | マッチするURI1回、refreshTimeout=null   | Equivalence - normal                 | setTimeout(750ms)でrepoChangeCallbackがスケジュールされる。clearTimeoutは呼ばれない | L53 false → L56-58 |
