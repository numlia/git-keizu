...
> Status: active
> Supersedes: -
> Signature: `BranchCleanupAncestry` / `BranchCleanupTreeDifference` / `BranchCleanupAheadBehind` / `BranchCleanupUpstream` / `BranchCleanupWorktree` / `BranchCleanupLastCommit` / `BranchCleanupRow` / `BranchCleanupResult` / `RequestLoadBranchCleanup` / `ResponseLoadBranchCleanup`
> Target Path: `src/types.ts`（branch cleanup 診断の型群と message union。実装後に行範囲へ更新）
> Test File: `tests/src/types.test.ts`

対応プラン §3.2 の fact union と診断 message の型契約を固定する。`unknown` と `notSelected`、`unset` と `gone`、`unused` と取得失敗（`unknown` / `null`）を同じ値へ潰さないことを型レベルで検証する。runtime validation は `web/branchCleanupPanel-test.md` S2、message routing は `src/gitGraphView-test/05-branch-cleanup-01.md` S33 の責務で、本 section は `@ts-expect-error` と代入可否だけを検証する。

| Case ID | Input / Precondition                                                                                                                                                                                                      | Perspective (Normal / Validation / Exception / External / Boundary / Type) | Expected Result                                                                                         | Notes                               |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| TC-092  | `"ancestor"` / `"notAncestor"` / `"unknown"` / `"notSelected"` を `BranchCleanupAncestry` へ代入し、`"safe"` も代入                                                                                                       | Type - ancestry literal union                                              | 4 literal の代入がコンパイルでき、`"safe"` は `@ts-expect-error` が有効（型エラーになる）               | 総合判定語を持たない                |
| TC-093  | `"same"` / `"different"` / `"unknown"` / `"notSelected"` を `BranchCleanupTreeDifference` へ代入し、`"differ"` も代入                                                                                                     | Type - treeDifference literal union                                        | 4 literal の代入がコンパイルでき、`"differ"` は `@ts-expect-error` が有効                               | ancestry と独立の union             |
| TC-094  | `{ kind: "known", ahead: 0, behind: 0 }` を `BranchCleanupAheadBehind` へ代入し、`ahead` を欠落させた literal も代入                                                                                                      | Type - aheadBehind known variant                                           | known の代入がコンパイルでき、`ahead` 欠落は `@ts-expect-error` が有効                                  | 数値対の必須化                      |
| TC-095  | `{ kind: "unknown" }` / `{ kind: "notSelected" }` を代入し、narrowing 後に `ahead` へアクセス                                                                                                                             | Type - aheadBehind 失敗 variant                                            | 双方の代入がコンパイルでき、narrowing 後の `ahead` アクセスは `@ts-expect-error` が有効                 | unknown と notSelected は別 variant |
| TC-096  | `{ kind: "present" }` / `{ kind: "gone" }`（`name` 欠落）を `BranchCleanupUpstream` へ代入                                                                                                                                | Type - upstream name 必須                                                  | 2 つとも `@ts-expect-error` が有効で、`name` 付き literal は代入がコンパイルできる                      | present / gone の name 必須         |
| TC-097  | `{ kind: "unset" }` / `{ kind: "unknown" }` を代入し、narrowing 後に `name` へアクセス                                                                                                                                    | Type - upstream name なし variant                                          | 双方の代入がコンパイルでき、narrowing 後の `name` アクセスは `@ts-expect-error` が有効                  | unset と unknown は別 variant       |
| TC-098  | `{ kind: "used", path: "/wt", isMain: false }` を `BranchCleanupWorktree` へ代入し、`path` / `isMain` を各々欠落                                                                                                          | Type - worktree used 必須 field                                            | used の代入がコンパイルでき、`path` 欠落・`isMain` 欠落はいずれも `@ts-expect-error` が有効             | -                                   |
| TC-099  | `{ kind: "unused" }` / `{ kind: "unknown" }` を代入し、narrowing 後に `path` へアクセス                                                                                                                                   | Type - worktree unused / unknown variant                                   | 双方の代入がコンパイルでき、narrowing 後の `path` アクセスは `@ts-expect-error` が有効                  | unused と unknown は別 variant      |
| TC-100  | `{ kind: "known", unixSeconds: 0 }` を `BranchCleanupLastCommit` へ代入し、`unixSeconds` を欠落させた literal も代入                                                                                                      | Type - lastCommit known variant                                            | known の代入がコンパイルでき、`unixSeconds` 欠落は `@ts-expect-error` が有効                            | -                                   |
| TC-101  | 全 field を持つ literal を `BranchCleanupRow` へ代入                                                                                                                                                                      | Type - row 完全 literal                                                    | 代入がコンパイルでき、各 fact field を対応する union 型として参照できる                                 | -                                   |
| TC-102  | `BranchCleanupRow` から `ancestry` を欠落させた object literal                                                                                                                                                            | Type - row 必須 field                                                      | `@ts-expect-error` が有効（型エラーになる）                                                             | fact の省略を防ぐ                   |
| TC-103  | `remotes: null` の row literal を代入し、`remotes` を欠落させた literal も代入                                                                                                                                            | Type - remotes null 許容・省略不可                                         | `null` の代入がコンパイルでき、field 欠落は `@ts-expect-error` が有効                                   | 失敗 null と省略の区別              |
| TC-104  | `isCurrent: null` の row literal を代入                                                                                                                                                                                   | Type - isCurrent null 許容                                                 | 代入がコンパイルできる                                                                                  | detached での不明を表現             |
| TC-105  | `{ kind: "ok", compareBranch: null, rows: [] }` を `BranchCleanupResult` へ代入                                                                                                                                           | Type - result ok variant                                                   | 代入がコンパイルでき、`kind === "ok"` で narrowing した分岐で `rows` / `compareBranch` へアクセスできる | 比較先なしの成功                    |
| TC-106  | `{ kind: "error", status: "fatal" }` を代入し、narrowing 後に `rows` へアクセス                                                                                                                                           | Type - result error variant                                                | 代入がコンパイルでき、narrowing 後の `rows` アクセスは `@ts-expect-error` が有効                        | 全体 error と空成功の区別           |
| TC-107  | `{ kind: "partial" }` を `BranchCleanupResult` へ代入                                                                                                                                                                     | Type - 未知の result kind                                                  | `@ts-expect-error` が有効（型エラーになる）                                                             | union 外の値                        |
| TC-108  | `{ command: "loadBranchCleanup", repo: "/r", requestId: 1, compareBranch: null }` を `RequestLoadBranchCleanup` へ代入し、`compareBranch` を欠落させた literal も代入                                                     | Type - request 必須 field                                                  | 完全 literal の代入がコンパイルでき、`compareBranch` 欠落は `@ts-expect-error` が有効                   | optional 化を防ぐ                   |
| TC-109  | `RequestLoadBranchCleanup` の値を `RequestMessage` union の変数へ代入                                                                                                                                                     | Type - request union 組み込み                                              | 代入がコンパイルでき、`command === "loadBranchCleanup"` で narrowing できる                             | message protocol への参加           |
| TC-110  | `{ command: "loadBranchCleanup", repo: "/r", requestId: 1, result: { kind: "ok", compareBranch: null, rows: [] } }` を `ResponseLoadBranchCleanup` と `ResponseMessage` へ代入し、`requestId` を欠落させた literal も代入 | Type - response 必須 field と union 組み込み                               | 完全 literal は双方へ代入がコンパイルでき、`requestId` 欠落は `@ts-expect-error` が有効                 | echo 用 field の必須化              |
| TC-111  | `switch` で `BranchCleanupAncestry` の 4 literal を網羅                                                                                                                                                                   | Type - exhaustive narrowing                                                | `default` 節で残余値を `never` 型の変数へ代入してもコンパイルできる（列挙漏れがない）                   | 表示分岐の網羅性の土台              |

### 失敗源インベントリ（include-or-justify）— Feature 055-03 追加分（S8）

| 失敗源                                                       | 対応ケースまたは除外理由                                                                                          |
| ------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| unknown / notSelected、unset / gone、unused / 失敗の同一値化 | TC-092、TC-093、TC-095、TC-097、TC-099、TC-103                                                                    |
| variant 固有 field の欠落・optional 化                       | TC-094、TC-096、TC-098、TC-100、TC-102、TC-103、TC-108、TC-110                                                    |
| union 外の値の受理                                           | TC-092、TC-093、TC-107                                                                                            |
| narrowing できない型設計                                     | TC-095、TC-097、TC-099、TC-105、TC-106、TC-111                                                                    |
| message union へ組み込まれない                               | TC-109、TC-110                                                                                                    |
| 各分岐の negative 側（有効な literal が拒否される）          | TC-101、TC-104、TC-105、TC-108                                                                                    |
| runtime の値検証・routing の誤り                             | excluded(`web/branchCleanupPanel-test.md` S2 と `src/gitGraphView-test/05-branch-cleanup-01.md` S33 の責務)       |
| 境界値（0 / minimum / maximum / +/-1 / empty / NULL）        | excluded(型定義に数値境界がない。`null` の許容 / 拒否は TC-103 / TC-104、`unixSeconds: 0` は TC-100 に含めて検証) |
| 外部依存の失敗・例外送出                                     | excluded(型定義のみで外部依存と throw 経路を持たない)                                                             |
...
- Exception: excluded(同上。throw 経路なし)
- External: excluded(外部依存なし)
- Boundary: excluded(数値境界が存在しない。`null` / `0` の代入可否は TC-100 / TC-103 / TC-104 に含めて検証)
- Type: TC-092〜TC-111
