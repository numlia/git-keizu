...
| TC-102  | `remoteSourceCache[repo]` に文字列でない値（object）が入っている                       | Validation - cache miss on non-string                                      | `typeof ... === "string"` が false でキャッシュヒットせず、`dataSource.getRemoteUrl(repo)` が再度呼ばれる | キャッシュ判定（旧 TC-020 系）。未カバー: 現行実装のキャッシュ判定は `typeof === "string"` ではなく `cachedRemoteSource !== undefined`（Feature 042 / active S21 TC-083 で修正済み）であり、object がキャッシュにある場合はヒットして `getRemoteUrl` は呼ばれない。本 Expected Result は superseded 済みの旧 TC-020 挙動を指しており、実装および active な S21 TC-083 と矛盾するため実装不能。実装済み S21 TC-083 が非文字列 object のキャッシュ挙動（ヒット）を担保する。 |
...
> Status: active
> Supersedes: -
> Signature: `public dispose(): void`（新規。disposed flag を追加し interval を clear/null 化）
> Target Path: `src/avatarManager.ts`（dispose 実装後に行範囲へ更新）

`AvatarManager` に disposed 状態と冪等な `dispose()` を追加する修正。dispose は interval を `clearInterval` して null 化し、以後 queue の `itemsAvailableCallback`、fetch 失敗時の retry/requeue、webview 通知から新規処理を開始しない（[13] の修正）。`context.subscriptions` への登録は `src/extension-test.md` S9（TC-036〜TC-037）の責務。進行中 HTTP request の強制 destroy は本対応の対象外。

| Case ID | Input / Precondition                                                  | Perspective (Normal / Validation / Exception / External / Boundary / Type) | Expected Result                                                                                                                                 | Notes                                    |
| ------- | --------------------------------------------------------------------- | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| TC-103  | queue に未処理 item があり interval 稼働中の状態で `dispose()` を呼ぶ | Normal - interval の停止                                                   | `clearInterval` が1回呼ばれ interval が null 化される。以後 `advanceTimersByTime(10000)` を進めてもアバター取得処理（fetch 経路）が開始されない | fake timer で観測（[13] の観測条件）     |
| TC-104  | `dispose()` を2回連続で呼ぶ                                           | Boundary - 冪等性                                                          | 2回目の呼び出しで例外が発生せず、`clearInterval` の追加呼び出しが0回である                                                                      | 冪等 dispose の契約                      |
| TC-105  | dispose 後に queue の `itemsAvailableCallback` を発火させる           | Validation - dispose 後の interval 再作成拒否                              | `setInterval` が呼ばれず（呼び出し0回）、`fetchAvatarsInterval()` の即時実行も行われない                                                        | S1 TC-002 の開始経路が dispose 後は無効  |
| TC-106  | dispose 後に進行中 fetch の error / rate-limit 経路が完了する         | External - dispose 後の retry/requeue 抑止                                 | `queue.add` による再キューが0回で、interval の再生成（`setInterval` 呼び出し）も0回である                                                       | error callback による timer 再作成の防止 |
| TC-107  | dispose 後にアバター取得結果の webview 通知タイミングが到来する       | Validation - dispose 後の通知抑止                                          | `view.sendMessage`（アバター応答通知）が呼ばれない                                                                                              | 新規処理の不開始                         |

### 失敗源インベントリ（include-or-justify）— Feature 045 追加分（S26）

| 失敗源                                                 | 対応ケースまたは除外理由                                                       |
| ------------------------------------------------------ | ------------------------------------------------------------------------------ |
| dispose 後も interval が停止しない                     | TC-103                                                                         |
| 多重 dispose による例外・二重解放                      | TC-104                                                                         |
| dispose 後の queue callback による interval 再作成     | TC-105                                                                         |
| dispose 後の fetch error による requeue / timer 再生成 | TC-106                                                                         |
| dispose 後の webview 通知                              | TC-107                                                                         |
| 進行中 HTTP request の即時中断漏れ                     | excluded(プラン対象外。timer と後続副作用の停止で本仕様を満たす)               |
| extension への登録漏れ                                 | excluded(`src/extension-test.md` S9 TC-036〜TC-037 で担保)                     |
| 通常運転時の queue 枯渇による interval 停止            | excluded(既存 S1〜S2 ほかの active section で担保済み。本変更で挙動を変えない) |

**失敗カテゴリ網羅（diversity floor）**:

- Validation: TC-105、TC-107
- Exception: excluded(dispose に throw 分岐を設けない。多重呼び出しの非例外は TC-104 で検証)
- External: TC-106
- Boundary: TC-104
- Type: excluded(dispose は引数を取らず、実行時の型分岐が存在しない)

数値・空値境界（0 / minimum / maximum / +/-1 / empty / NULL）は、本セクションの対象が disposed 状態遷移の契約であり数値境界を持たないため対象外とする（意味のある境界は多重 dispose の TC-104 で充足）。

**失敗系/正常系比（煙感知器）**: 正常系1件（TC-103）、失敗系4件（TC-104〜TC-107）、比4.0。
