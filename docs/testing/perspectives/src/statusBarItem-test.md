...
| ------- | ------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| TC-001  | `createStatusBarItem` がモック `StatusBarItem` を返し、`context.subscriptions.push` が利用可能    | Normal - standard                                                          | `vscode.window.createStatusBarItem` が `(vscode.StatusBarAlignment.Left, 1)` で 1 回呼ばれる。返却された項目の `text` が `"Git Keizu"`、`tooltip` が `"View Git Keizu"`、`command` が `"git-keizu.view"` に設定され、`context.subscriptions.push` が同じ項目で 1 回呼ばれる | L10-L14 |
...
| Case ID | Input / Precondition                                                | Perspective (Normal / Validation / Exception / External / Boundary / Type) | Expected Result                                                                                                 | Notes                     |
| ------- | ------------------------------------------------------------------- | -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------- |
| TC-004  | `numRepos = 3`、`refresh` を `vi.spyOn(instance, "refresh")` で監視 | Normal - standard                                                          | 内部状態 `numRepos` が `3` に更新され、`refresh()` が引数なしで 1 回呼ばれる                                    | L18-L19                   |
...
| Case ID | Input / Precondition                                                                                                           | Perspective (Normal / Validation / Exception / External / Boundary / Type) | Expected Result                                                                                                   | Notes                       |
| ------- | ------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | --------------------------- |
| TC-007  | `getConfig().showStatusBarItem()` が `true` を返し、内部 `numRepos = 1`                                                        | Normal - branch                                                            | `statusBarItem.show()` が 1 回呼ばれ、`statusBarItem.hide()` は呼ばれない                                         | L23-L24。最小の表示側境界値 |
...
| TC-009  | `getConfig().showStatusBarItem()` が `false` を返し、内部 `numRepos = 5`                                                       | Normal - config disabled                                                   | `statusBarItem.hide()` が 1 回呼ばれ、`statusBarItem.show()` は呼ばれない。リポジトリ数が正でも設定値が優先される | L23-L26                     |
