...
| Case ID | Input / Precondition                                                                                                  | Perspective (Normal / Validation / Exception / External / Boundary / Type) | Expected Result                                                                                                                 | Notes                      |
| ------- | --------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| TC-243  | `branch --show-current` が `"main"`、`branch.main.remote` が `"upstream"`、`branch.main.merge` が `"refs/heads/main"` | Normal - upstream あり                                                     | `{ kind: "upstream", target: { remoteName: "upstream", branchName: "main" } }` を返す                                           | origin 固定でない upstream |
...
`push(repo)` の `["push", "--set-upstream", "origin", "HEAD"]` 固定を廃し、upstream ありは `["push", <remote>, <branch>]`、選択確定後の新規 upstream 登録だけ `["push", "--set-upstream", <remote>, "HEAD"]` を使う変更。`push()` と origin 固定 args を除去するため、`["push"]`（引数なし）を現行仕様として固定していた旧 S10 を supersede する。旧 S10 が同一セクションで扱っていた `pull()` は本変更の対象外で挙動を変えないため、S44 として active のまま引き継ぐ。`fetch()` / `pushTag()` は変更しない。

| Case ID | Input / Precondition                                                                                                                                       | Perspective (Normal / Validation / Exception / External / Boundary / Type) | Expected Result                                                                                                                                                           | Notes                      |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| TC-254  | `pushToUpstream(repo, { remoteName: "upstream", branchName: "main" })`、git が exit 0                                                                      | Normal - upstream への明示 Push                                            | `cp.spawn` が args `["push", "upstream", "main"]` で 1 回呼ばれ、`null` を返す。args に `"--set-upstream"` と `"HEAD"` を含まない                                         | origin 固定の除去          |
...
| TC-258  | `pushToUpstream(repo, { remoteName: "-evil", branchName: "main" })`                                                                                        | Validation - unsafe remote 名                                              | `cp.spawn` の call count が 0。非 null のエラー文字列を返す                                                                                                               | 実行前 guard               |
| TC-259  | `pushToUpstream(repo, { remoteName: "origin", branchName: "feature..x" })`                                                                                 | Validation - 不正 branch 名                                                | `cp.spawn` の call count が 0。非 null のエラー文字列を返す                                                                                                               | 実行前 guard               |
| TC-260  | `pushWithUpstream(repo, "")`                                                                                                                               | Boundary - empty remote 名                                                 | `cp.spawn` の call count が 0。非 null のエラー文字列を返す                                                                                                               | 空 remote 名               |
| TC-261  | 実 Git 一時 repository。bare remote を `origin` と `upstream` の 2 つ登録し、current branch の upstream を `upstream` に設定して `pushToUpstream()` を実行 | Normal - non-origin upstream への Push（実 Git）                           | `upstream` 側 bare repository の `refs/heads/<branch>` hash が local の HEAD hash と一致し、`origin` 側の `refs/heads/<branch>` は実行前のまま（未作成 / 変化なし）である | 誤 remote への Push を検出 |
